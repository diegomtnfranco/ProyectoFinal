...
function TotemCheckOut() {
  ...
  useEffect(() => {
    const fetchQR = async () => {
      try {
        ...
        setQrImageBase64(response.data?.checkOut?.qrUrl || null);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-emerald-600 flex flex-col items-center justify-center p-8 text-center font-sans">
      <div className="bg-white rounded-3xl shadow-2xl p-12 max-w-3xl w-full flex flex-col items-center">
        <div className="bg-emerald-100 p-6 rounded-full mb-8 text-emerald-600 animate-pulse">
          <ScanLine className="w-20 h-20" strokeWidth={1.5} />
        </div>

        <h1 className="text-5xl md:text-6xl font-extrabold text-gray-900 mb-6 tracking-tight">¡Hasta luego!</h1>
        <p className="text-2xl md:text-3xl text-gray-600 mb-12 leading-relaxed">
          Para salir, abrí la cámara de tu celular y <br/> <strong>escaneá este código QR</strong>.
        </p>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-lg border-4 border-gray-50 flex items-center justify-center min-h-[350px] min-w-[350px]">
          {error ? (
            ...
            <img src={qrImageBase64} alt="QR de Salida" className="w-80 h-80 object-contain" />
          ) : (
